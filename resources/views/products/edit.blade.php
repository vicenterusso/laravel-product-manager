...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mt-8">
        <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-white shadow px-4 py-5 sm:rounded-lg sm:p-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="number" name="quantity" id="quantity" value="{{ old('quantity', $product->quantity) }}" step="0.01" min="0" required
            ...
            <div>
                ...
                @if($product->photo)
                    <div class="mt-2 mb-3">
                        <img src="{{ $product->photo_url }}" alt="{{ $product->name }}" class="h-32 w-32 rounded object-cover">
                    </div>
                @endif
